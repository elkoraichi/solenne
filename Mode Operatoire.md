...
| 3 — Séjours ★ | 6 | ✅ Validé | 134 | 134 | 0 | 27/08/2026 |
...
Lot 3 : `OCCUP` livré à **34 cas sur 34**, en deux arrêts (`OCCUP-A` contrat, `OCCUP-B` exercice) ; `AVAIL` ★ livré à **35 cas sur 35**, en trois arrêts (`AVAIL-A` contrat et R1→R4, `AVAIL-B` R5→R8, `AVAIL-C` combinaisons) ; `POLICY` livré à **16 cas sur 16** (`POLICY-012` fermé par `STAYREQ`, seul module qui crée une vraie demande de séjour) ; `STAYREQ` livré à **20 cas sur 20**, en deux arrêts (`STAYREQ-A` Server Actions et composition R8, `STAYREQ-B` assistant en trois étapes et disponibilité en direct) ; `STAYDEC` ★ livré à **19 cas sur 19**, en deux arrêts (`STAYDEC-A`, Opus, la revalidation en transaction sérialisable et la grille C1→C6, qui a ajouté le cas `STAYDEC-C06` à la fiche ; `STAYDEC-B`, Sonnet, file d'attente, écran de décision, refus, contre-proposition et sécurité `S02`/`S06`) ; `STAY` livré à **10 cas sur 10**, en un seul arrêt (Sonnet) — création directe, annulation des deux côtés, clôture automatique, suggestion de libération. Les six modules du lot sont clos. **Lot 3 validé le 27/08/2026** : régression complète (852 Vitest, `tsc`/`eslint` muets) et campagne Playwright aux trois tailles (459 passés, 10 ignorés) au vert, un défaut de test daté corrigé au passage (`CAL-007`, entrée 1.19). Jugement visuel **L2** sollicité auprès de Yassine sur les captures de `Rapports/apercus-lot3/`. Reste `DEPLOY`, dernier module de la vague 1.
...
| 1.19 | 27/08/2026 | **Lot 3 clos.** Régression complète rejouée aux trois tailles imposées par le lot (mesure M2) : **852 Vitest (~54 s)**, `tsc` et `eslint` muets, **459 Playwright au vert, 10 ignorés** (320/768/1440, ~1 min 20 avec le build de production). Un défaut trouvé et corrigé : **`CAL-007`** (module `CAL`, lot 2) visait le mois à `aujourd'hui + 21 jours` en y cherchant le séjour de Solenne du jeu de démonstration, qui tombe en réalité à **+40 jours** — les deux ne se retrouvent dans le même mois calendaire que certains jours du mois (`+21` et `+40` jours après le 26/08 tombent dans deux mois différents, ce qui n'était pas le cas les jours où le lot 2 avait été validé). Le test visait donc le mauvais mois selon la date d'exécution — un défaut dormant depuis le lot 2, jamais un défaut introduit par le lot 3. Corrigé en visant `+40` jours, l'écart réel du jeu de démonstration, plutôt que la constante partagée avec un autre cas. Aucun défaut d'application. Captures ajoutées pour la validation visuelle (`Rapports/apercus-lot3/` : `/sejours` côté ami et côté Solenne, `/gerer`) — **jugement visuel L2 sollicité auprès de Yassine**. **Lot 3 validé sur les 10 critères du §11.1. Reste `DEPLOY`, dernier module de la vague 1** |
