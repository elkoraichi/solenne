...
| 3 — Séjours ★ | 6 | 🟦 4 modules livrés · `STAYDEC` ★ à l'arrêt A · `STAY` restant | 134 | 113 | 0 | — |
...
Lot 3 : `OCCUP` livré à **34 cas sur 34**, en deux arrêts (`OCCUP-A` contrat, `OCCUP-B` exercice) ; `AVAIL` ★ livré à **35 cas sur 35**, en trois arrêts (`AVAIL-A` contrat et R1→R4, `AVAIL-B` R5→R8, `AVAIL-C` combinaisons) ; `POLICY` livré à **16 cas sur 16** (`POLICY-012` fermé par `STAYREQ`, seul module qui crée une vraie demande de séjour) ; `STAYREQ` livré à **20 cas sur 20**, en deux arrêts (`STAYREQ-A` Server Actions et composition R8, `STAYREQ-B` assistant en trois étapes et disponibilité en direct) ; `STAYDEC` ★ **à l'arrêt A**, 8 cas sur 19 — la revalidation en transaction sérialisable et la grille C1→C6, qui a ajouté le cas `STAYDEC-C06` à la fiche. Restent l'arrêt `STAYDEC-B` (file d'attente, écran de décision, refus, contre-proposition) et `STAY`.
